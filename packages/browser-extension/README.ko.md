...
2. ChatGPT를 엽니다.
3. 의도적으로 닫히지 않은 Markdown 코드 펜스를 만들도록 요청합니다.
4. assistant 응답 근처에 `Markdown fence issue` 배지가 나타나는지 확인합니다.
5. 배지를 클릭합니다.
6. 수정 미리보기 패널이 열리는지 확인합니다.
7. `Copy fixed Markdown` 버튼으로 수정된 Markdown이 복사되는지 확인합니다.
